...
if [[ -z "${BITRISE_PULL_REQUEST}" ]]; then
    ...
    ./gradlew clean gnagCheck build jacocoDebugReports
else
    echo "---> Making build for Pull Request"
    # In the env variable BITRISE_PULL_REQUEST Bitrise CI provides the ID of the PR
    ./gradlew clean gnagReport -PauthToken="${GITHUB_TOKEN}" -PissueNumber="${BITRISE_PULL_REQUEST}" build jacocoDebugReports
fi